    ...
    <div id={props.id}>
      <div className="group rounded overflow-hidden shadow-lg bg-card flex flex-col h-full transition-transform duration-200 hover:-translate-y-1 hover:shadow-xl hover:ring-1 hover:ring-primary/30">
        ...
        <div className="px-6 py-8 w-auto size-40">
          <div className="font-bold text-xl text-green-800">{props.title}</div>
        ...
        {showDetails && (
          <div id={props.id} className="px-6 pb-4 mt-auto flex flex-col items-center justify-center gap-7 ">
            {props.url ? (
              <a id={props.id} href={props.url} target="_blank" rel="noreferrer" className="text-base text-black underline">Voir le projet</a>
            ) : (
              <span className="text-base text-black">Mise à jour en cours ...</span>
            )}
            {props.gitHub && (
              <a id={props.id} href={props.gitHub} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-base text-black underline">
